...
###############################################################################
balance_score <- function(X, Y = NULL, family,
  ...
  bin_prev_by_index <- setNames(bin_prev, bin_idx)
  ...
  if (is.null(rareFeature.index)) {
    ...
    supplied_prev <- unname(bin_prev_by_index[as.character(rare_idx)])
...
glm.coef.estimate <- function(X,
  ...
  if (family$family == "gaussian" && family$link == "identity") {
    results <- lm.wfit(x = X, y = Y, w = weights)
...
###############################################################################
calculate.nm <- function(X, Y, ddL.plt, linear.predictor, criterion, 
                              bl, DN = 1){
...
###############################################################################
poi.prob.estimated <- function(nm,
                               index.plt,
                               pi.plt,
                               n.ssp,
                               N,
...
ddL <- function (eta, X, weights = 1, family) {
...
cov.score <- function (eta, X, Y, weights = 1, family) {
  variance <- family$variance
  linkinv  <- family$linkinv
  
  temp <- (Y - linkinv(eta))^2
  
  cov.score <- t(X) %*% (X * (temp * weights))
  return(cov.score)
}
###############################################################################
pilot.estimate <- function(inputs, ...){
  ...
  if (family[["family"]] %in% c('binomial', 'quasibinomial')){
    ...
    beta.plt <- glm.coef.estimate(X = x.plt, Y = Y.plt, 
                                  weights = fit.weights.plt,
                                  family = family, ...)$beta
    linear.predictor.plt <- as.vector(x.plt %*% beta.plt) # dimension: n.plt
    
    ddL.plt <- (1/N) * ddL(eta = linear.predictor.plt,
                          X = x.plt,
                          weights = fit.weights.plt,
                          family = family)
    cov.score.plt <- (n.plt / (N^2)) * cov.score(eta = linear.predictor.plt,
                                                 x.plt,
                                                 Y.plt,
                                                 weights = fit.weights.plt^2,
                                                 family = family)
    ddL.plt.design <- (1/N) * ddL(eta = linear.predictor.plt,
                                  X = x.plt,
                                  weights = info.weights.plt,
                                  family = family)
    # ddL.plt.design is for approximating full ddL so we use weights.
    linear.predictor <- linkinv(X %*% beta.plt) # dimension: N
    ddL.plt.inv <- solve(ddL.plt)
    cov.plt <- (1/n.plt) * ddL.plt.inv %*% cov.score.plt %*% ddL.plt.inv
    ...
    cov.score.plt <- (n.plt / (N^2)) * cov.score(
      eta = linear.predictor.plt,
      x.plt,
      Y.plt,
      weights = fit.weights.plt^2,
...
subsampling <- function(inputs,
  ...
  nm <- calculate.nm(X, Y, ddL.plt, linear.predictor, criterion, 
  ...
  get_poi_prob_results <- function(nm_values,
                                   ...
                                   population_size) {
    if (poi.method == "exact") {
      return(poi.prob.exact(nm_values, target_n, population_size))
    }
    poi.prob.estimated(
      nm = nm_values,
  ...
  if (balance.Y.all) {
    # include all Y=1 and do subsampling for Y=0
    N0 <- N - sum(Y)
...
subsample.estimate <- function(inputs,
  ...
  if (sampling.method == 'poisson') {
    ddL.ssp <- (1/N) * ddL(linear.predictor.ssp,
                           x.ssp,
                           weights = w.ssp,
                           family = family)
    cov.score.ssp <- (n.ssp / (N^2)) * cov.score(linear.predictor.ssp,
                                                 x.ssp,
                                                 y.ssp,
                                                 weights = w.ssp^2,
                                                 family = family)
  ...
  cov.ssp <- (1/n.ssp) * ddL.ssp.inv %*% cov.score.ssp %*% ddL.ssp.inv 
...
###############################################################################
uniform.estimate <- function(inputs, ...){
  X <- inputs$X
  Y <- inputs$Y
  n.uni <- inputs$n.uni
  family <- inputs$family
  ...
  if (family[["family"]] %in% c('binomial', 'quasibinomial')){
    ...
    beta.uni <- glm.coef.estimate(X = X.uni, Y = Y.uni, 
                                  weights = fit.weights.uni,
                                  family = family, ...)$beta
    linear.predictor.uni <- as.vector(X.uni %*% beta.uni) # dimension: n.uni
    
    # ddL.uni and cov.score.uni: the pilot estimation of score^2 and information
    if (objective.weight == "weighted") {
      ddL.uni <- ddL(eta = linear.predictor.uni,
                     X = X.uni,
                     weights = (1 / (varphi.uni * n.uni)),
                     family = family)
      cov.score.uni <- cov.score(eta = linear.predictor.uni,
                         X.uni,
                         Y.uni,
    ...
    cov.uni <- (1 / n.uni) * ddL.uni.inv %*% cov.score.uni %*% ddL.uni.inv
  } else {
    ...
    # ddL.uni and cov.score.uni: the pilot estimation of gradient^2 and information
    if (objective.weight == "weighted") {
      ddL.uni <- ddL(eta = linear.predictor.uni,
                     X = X.uni,
                     weights = (1 / (varphi.uni * n.uni)),
...
###############################################################################
glm.control <- function(alpha = 0, b = 2, poi.method = c("exact", "estimated"), ...)
{
...
###############################################################################
summary.ssp.rareF.glm <- function(object, ...) {
  ### collect results
  ...
  if (family.name %in% c("binomial", "quasibinomial")) {
    ...
    y_summary <- data.frame(
      Sample = c("Pilot", "Subsample", "Combined union", "Full data"),
      Size = c(n.plt.actual, n.ssp.actual, n.cmb.actual, N),
      Y_sum = c(object$Y.count.plt,
                object$Y.count.ssp,
                object$Y.count.cmb.union,
                object$full.Y.sum),
      Y_mean = c(
        round(object$Y.proportion.plt, 4),
        round(object$Y.proportion.ssp, 4),
        round(object$Y.proportion.cmb.union, 4),
        round(object$full.Y.mean, 4)
      ),
      Rare_row_rate = c(
        safe_pct(length(object$rows.with.rare.plt), n.plt.actual),
        safe_pct(length(object$rows.with.rare.ssp), n.ssp.actual),
  ...
  if (!is.null(object$coef.cmb.union) && !is.null(object$cov.cmb.union)) {
    cat("\nCombined-Union Coefficients:\n\n")
    coef_union <- object$coef.cmb.union
    se_union <- sqrt(diag(object$cov.cmb.union))
    coef_union_table <- data.frame(
      Estimate = round(coef_union, digits = 4),
      `Std. Error` = round(se_union, digits = 4),
      `z value` = round(coef_union / se_union, digits = 4),
      `Pr(>|z|)` = format_p_values(
        2 * (1 - pnorm(abs(coef_union / se_union))),
        threshold = 0.0001
      ),
      check.names = FALSE
    )
    rownames(coef_union_table) <- names(coef_union)
    print(coef_union_table)
  }
}
